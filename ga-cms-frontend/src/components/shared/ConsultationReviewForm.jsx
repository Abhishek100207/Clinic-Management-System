...
/**
 * ConsultationReviewForm - A premium, self-contained and shareable review form 
 * component for patient consultations.
 * 
 * Props:
 * - appointmentId: ID of the appointment being reviewed (optional, falls back to demo).
 * - doctorName: Name of the doctor (optional, falls back to demo).
 * - patientName: Name of the patient (optional, falls back to demo).
 * - date: Date of consultation (optional, falls back to today).
 * - onSubmit: Callback function when form is submitted (optional, triggers mock API call).
 */
const ConsultationReviewForm = ({ 
  appointmentId = 'APT-98241', 
  ...
  // Comments visibility toggles (makes the UI cleaner by allowing collapsible comments per rating)
  ...
  const handleRatingChange = (key, val) => {
    ...
    // Clear validation error if any rating is set
    setValidationError('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if at least the primary ratings (Consultation and Doctor) are filled
    ...
    try {
      if (onSubmit) {
        ...
        // Mock API transmission call for standalone use
        console.log('Sending Review Payload to API Endpoint [/api/appointments/reviews/]:', payload);
        await new Promise(resolve => setTimeout(resolve, 1500));
      }
      setIsSubmitted(true);
    } catch (err) {
      setValidationError(err.message || 'An error occurred while submitting feedback. Please try again.');
    } finally {
    ...
    return (
      <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 text-white rounded-[2.5rem] p-8 md:p-12 text-center shadow-2xl space-y-6 my-8 animate-in fade-in duration-500">
        ...
        <div className="pt-6 border-t border-slate-800/80 max-w-sm mx-auto">
          ...
          <p className="text-xs text-slate-500 mt-1">Doctor: <span className="text-slate-400 font-semibold">{doctorName}</span></p>
        </div>
        <button 
  ...
  return (
    <div className="max-w-3xl mx-auto bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl my-8 animate-in fade-in duration-300">
      ...
      <div className="relative bg-gradient-to-r from-blue-900 via-indigo-950 to-slate-900 p-8 md:p-10 border-b border-slate-800">
        ...
        {/* Info Card */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-8 mt-8 border-t border-slate-800/80">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-1">Doctor</span>
            <span className="text-sm font-bold text-white flex items-center gap-1.5"><Heart size={14} className="text-rose-500" /> {doctorName}</span>
          </div>
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-1">Patient</span>
            <span className="text-sm font-bold text-slate-300">{patientName}</span>
          </div>
          <div>
