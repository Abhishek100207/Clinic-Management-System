...
const SignInPage = () => {
  ...
  const handleCredentials = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await authApi.loginRequestOtp(email, password);
      setStep('otp');
      startResendTimer();
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4"
      ...
      <button onClick={() => step === 'otp' ? setStep('credentials') : navigate('/')}
        ...
        <div className="rounded-3xl overflow-hidden border border-white/10"
          ...
          <div className="flex px-8 pt-6 gap-2">
            {['credentials','otp'].map((s, i) => (
              <div key={s} className="flex-1 flex flex-col gap-1">
                <div className={`h-1 rounded-full transition-all duration-300 ${step === s || (i === 0) ? 'bg-blue-500' : 'bg-white/10'}`}
                  style={{ background: i === 0 ? '#3b82f6' : step === 'otp' ? '#3b82f6' : 'rgba(255,255,255,0.1)' }} />
                <p className="text-white/30 text-[10px]">{i === 0 ? 'Credentials' : 'OTP Verify'}</p>
              </div>
            ))}
          </div>
          ...
          <div className="px-8 py-6">
            {step === 'credentials' ? (
                ...
                <form onSubmit={handleCredentials} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      placeholder="you@gaclinic.com" autoFocus
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <button type="button" className="text-xs text-blue-400 hover:text-blue-300 transition-colors">Forgot?</button>
                    </div>
                  ...
                  <button type="submit"
                    className="w-full py-3.5 rounded-xl text-white font-bold text-sm mt-1 hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-blue-900/40"
                    style={{ background: 'linear-gradient(135deg,#1d4ed8,#0ea5e9)' }}>
                    Continue — Get OTP →
                  </button>
                </form>
              ...
              <form onSubmit={handleVerifyOtp} className="flex flex-col gap-5">
                <div className="rounded-2xl px-4 py-3 text-center border border-blue-400/20"
                  ...
                  <p className="text-blue-300 text-xs">OTP sent to <span className="font-bold text-white">{email}</span></p>
                </div>
                ...
                <button type="submit"
                  className="w-full py-3.5 rounded-xl text-white font-bold text-sm hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-blue-900/40"
                  style={{ background: 'linear-gradient(135deg,#1d4ed8,#0ea5e9)' }}>
                  Verify & Sign In →
                </button>
            ...
            <p className="text-center text-white/20 text-xs mt-5">
              No account?{' '}
              <button onClick={() => navigate('/signup')} className="text-blue-400 hover:text-blue-300 font-semibold transition-colors">Sign Up</button>
            </p>
          </div>
        </div>
...
const ErrorBox = ({ msg }) => {
  ...
  return (
    <div className={`flex items-start gap-2 rounded-xl px-3 py-2.5 border ${isDev ? 'border-blue-400/20' : 'border-red-400/20'}`}
      ...
      <p className={`text-xs leading-relaxed ${isDev ? 'text-blue-300 font-bold' : 'text-red-400'}`}>{msg}</p>
    </div>
  );
};
